...
func parseCommandLineArgs() *cliOptions {
	...
	pflag.StringVar(&destMACArg, "dest-mac", "b4:2e:99:a6:a5:47",
		"MAC address of the destination IP or gateway (required)")

	pflag.StringVar(&destIPArg, "dest-ip", "",
		"Destination IP address (required)")

	pflag.StringSlice("dest-ips", destIPsArg, "Destination IP addresses with port (required)")
	cliOps.destIPsArg = destIPsArg

	pflag.Uint16Var(&cliOps.destPort, "dest-port", defaultDestPort, "Destination port")
